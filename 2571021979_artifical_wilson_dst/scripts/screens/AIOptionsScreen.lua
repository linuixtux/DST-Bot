...
local AIControlsScreen = require "screens/aicontrolsscreen"
...
local AIOptionsScreen = Class(Screen, function(self, modname, colorname_vectors, outlined_anims)
	Screen._ctor(self, "AIOptionsScreen")
	...
	self.bg = self.proot:AddChild(TEMPLATES.RectangleWindow(619, 359, "AI Options", close_button))
	...
	self.key_binds_button = self.proot:AddChild(TEMPLATES.StandardButton(
		function()
			TheFrontEnd:PushScreen(AIControlsScreen(self.modname, self))
			self:Hide()
		end,
		"Keybinds",
		{100, 50}))
...
function AIOptionsScreen:SetColorMode(mode)
...
function AIOptionsScreen:SetUpFocusHookups()
    if TheInput:ControllerAttached() then
...
function AIOptionsScreen:OnFocusMove(dir, down)
	...
	return AIOptionsScreen._base.OnFocusMove(self, dir, down)
end

function AIOptionsScreen:GetHelpText()
...
function AIOptionsScreen:OnRawKey(key, down)
	if AIOptionsScreen._base.OnRawKey(self, key, down) then return true end
...
function AIOptionsScreen:OnControl(control, down)
	if AIOptionsScreen._base.OnControl(self,control, down) then return true end
...
function AIOptionsScreen:Close()
	self.active = false
	self.callbacks.save()
...
function AIOptionsScreen:OnUpdate(dt)
	if self.active then
		SetPause(true)
	end
end

function AIOptionsScreen:OnBecomeActive()
	AIOptionsScreen._base.OnBecomeActive(self)
	-- Hide the topfade, it'll obscure the pause menu if paused during fade. Fade-out will re-enable it
	TheFrontEnd:HideTopFade()
end

return AIOptionsScreen
