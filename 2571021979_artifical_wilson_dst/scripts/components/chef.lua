local Chef = Class(function(self, inst)
   ...
   self.neverMake = {"wetgoop", "fishtacos", "unagi",
                     ...
                     "ratatouille", "monsterlasagna", "powcake", "bonestew"}
...
function Chef:MakeSomethingGood(cooker, onsuccess, onfail)
   ...
   action:AddSuccessAction(function() if math.random() < .1 then self.inst.components.talker:Say("I'm Gordon Ramsay") end if onsuccess then onsuccess() end end)
...
function Chef:WhatCanIMake()
   ...
   local prefers_to_eat = function(food)
      if food.foodtype and self.inst.components.eater.preferseating then
         for i,v in ipairs(self.inst.components.eater.preferseating) do
            -- This can be either a FOODTYPE, or a FOODGROUP...
            if type(v) == "string" then
               if food.foodtype == v then
                  return true
               end
            elseif type(v) == "table" then
               if v.types ~= nil then
                  for i2,v2 in ipairs(v.types) do
                     if food.foodtype == v2 then
                        return true
                     end
                  end
               end
   ...
   for i,j in pairs(combo_ingredients) do
      ...
      for k,v in pairs(cooking.recipes.cookpot) do
         ...
         local prefers = prefers_to_eat(v)

         -- Don't try anything with more than 1 monster item.
         local too_much_monster = (j.tags and j.tags.monster and j.tags.monster >= 2) or false

         -- Meatballs have a chance with 4 meats....but that will likely just give bone stew
         local too_much_meat = (j.tags and j.tags.meat and j.tags.meat > 2) or false

         -- Dont' test for things on the ignore list, or things that might make monster lasagna
         if prefers and not ignore(v.name) and not too_much_monster and not too_much_meat then
            ...
            if v.test("cookpot", j.names, j.tags) then
               ...
               -- Don't cook anything we can't/wont eat
   ...
   if self.inst:HasTag("AIDebugPrint") then
      print("We can make: ")
