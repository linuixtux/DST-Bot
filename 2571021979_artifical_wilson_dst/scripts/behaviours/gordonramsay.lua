MasterChef = Class(BehaviourNode, function(self, inst)
   ...
   self.openedCooker = function(inst, data)
      if not self.currentCooker then return end
      if self.currentCooker and data.container ~= self.currentCooker then return end
...
function MasterChef:Visit()
    ...
    if self.status == READY then
      ...
      if not cooker.components.container.canbeopened then
         ...
         -- need to harvest it
         if cooker.components.stewer:IsCooking() then
            print("Because it's still cooking!")
            self.status = FAILED
            return
         end

         if cooker.components.stewer:IsDone() or cooker.components.stewer:IsSpoiling() then
         ...
         print("Can't open cooker for some reason. Maybe someone else has it open?")
         -- We can't open it and it's not done. Maybe someone else has it open?
         self.status = FAILED
         return
      end
