SelfPreservation = Class(BehaviourNode, function(self, inst, hunterparams, see_dist, safe_dist, fn, runhome)
   BehaviourNode._ctor(self, "SelfPreservation")
...
function SelfPreservation:__tostring()
   return string.format("SelfPreservation %f from: %s", self.safe_dist, tostring(self.hunter))
end

function SelfPreservation:GetRunAngle(pt, hp)
   ...
   --print(string.format("SelfPreservation:GetRunAngle me: %s, hunter: %s, run: %2.2f", tostring(pt), tostring(hp), angle))
...
function SelfPreservation:Visit()

   if self.status == READY then
      self.hunter = FindEntity(self.inst, self.see_dist, nil, nil, {"INLIMBO", "notarget"}, self.hunterparams)

      print("Found target: " .. tostring(self.hunter))
   ...
   if self.status == RUNNING then
      if not self.hunter or not self.hunter:IsValid() then
          self.status = FAILED
