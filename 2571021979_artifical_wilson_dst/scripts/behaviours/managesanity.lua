ManageSanity = Class(BehaviourNode, function(self, inst, target_max, target_min)
   BehaviourNode._ctor(self, "ManageSanity")
   self.inst = inst

   self.target_max = target_max or .9
   self.target_min = target_min or .5

   self.midway = (self.target_max + self.target_min) / 2
end)
...
function ManageSanity:Visit()
    ...
    if self.status == READY then
      ...
      if currentSanity < self.midway and self.inst.components.prioritizer:OnIgnoreList("petals") then
         self.inst.components.prioritizer:RemoveFromIgnoreList("petals")
      elseif currentSanity >= self.target_max and not self.inst.components.prioritizer:OnIgnoreList("petals") then
      ...
      if currentSanity >= math.max(0,self.target_min) then
         self.status = FAILED
         return
      end
